...
func testInfinityGlobalConfigurationIntegration(t *testing.T, client InfinityClient) {
	...
	fullConfig := test.LoadTestFolder(t, "resource_infinity_global_configuration_full_integration")
	minConfig := test.LoadTestFolder(t, "resource_infinity_global_configuration_min")
	...
	fullChecks := resource.ComposeTestCheckFunc(
		...
		resource.TestCheckResourceAttr("pexip_infinity_global_configuration.global_configuration-test", "transcript_mode_vmr_default", "besteffort"),
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Step 2: Update to min configuration (empty resource block — all schema defaults).
			{
				Config: minConfig,
				Check:  minChecks,
			},
			// Step 3: Destroy — triggers Delete which resets all fields to API defaults and
			// clears any references to related resources.
			{
				Config:  minConfig,
				Destroy: true,
			},
			// Step 4: Re-create with min configuration.
			{
				Config: minConfig,
				Check:  minChecks,
			},
			// Step 5: Update to full configuration with all related resources.
			{
				Config: fullConfig,
				Check:  fullChecks,
			},

resource "infinity_mjx_exchange_deployment" "test_min" {
...
resource "infinity_mjx_exchange_deployment" "test_full" {
  name                     = "tf-test mjx-exchange-deployment full"
  service_account_username = "exchange-service@example.com"
  service_account_password = "test-password"
}

resource "infinity_mjx_exchange_autodiscover_url" "test_full" {
  ...
  exchange_deployment = infinity_mjx_exchange_deployment.test_full.id
}

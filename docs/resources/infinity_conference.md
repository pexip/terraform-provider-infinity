...
- `enable_chat` (String) Enables relay of chat messages between conference participants using supported clients such as the Pexip apps. You can use this option to override the global configuration setting. Valid choices: default, yes, no.
...
- `non_idp_participants` (String) Determines whether participants attempting to join from devices other than Pexip apps (for example, SIP or H.323 endpoints) are permitted to join the conference when authentication is required. Disallow all: these devices may not join the conference. Allow if trusted: these devices may join the conference if they are locally registered. Valid choices: allow_if_trusted, disallow_all.
